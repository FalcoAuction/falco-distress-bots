...
def _parse_mmddyyyy(s: str) -> str | None:
    """
    Convert M/D/YYYY into ISO YYYY-MM-DD
    """
    if not s:
        return None
...
def _extract_table_rows(html: str, base_url: str) -> list[dict]:
    """
    Extract rows from the listings table.
    Each row includes:
      sale_date, continuance_date, city, address, zip, county, trustee, listing_url
    """
    soup = BeautifulSoup(html, "html.parser")
    text = soup.get_text(" ", strip=True)

    # The site renders a table; easiest is to walk all "View Listing" anchors,
    # then read their parent row text nearby.
    out: list[dict] = []

    for a in soup.select('a[href*="Foreclosure-Listing.aspx"][href]'):
        listing_url = urljoin(base_url, a["href"])

        # The anchor is inside a row; climb to nearest <tr> if possible
        tr = a.find_parent("tr")
        if tr:
            cells = [c.get_text(" ", strip=True) for c in tr.find_all(["td", "th"])]
        else:
            # fallback: try parent container text
            cells = [a.find_parent().get_text(" ", strip=True)]

        row_text = " | ".join([c for c in cells if c])

        # Heuristic parse: the main page shows:
        # Sale Date, Continuance Date, City, Address, Zip Code, County, Firm/Trustee, View Listing
        # We'll grab first 7 parsable chunks by scanning for mm/dd/yyyy twice then fields.
        parts = [p.strip() for p in re.split(r"\s{2,}|\|", row_text) if p.strip()]

        # Find the first two mm/dd/yyyy in the row
        dates = [p for p in parts if _DATE_RE.match(p)]
        sale_date_raw = dates[0] if len(dates) >= 1 else None
        cont_date_raw = dates[1] if len(dates) >= 2 else None

        # After dates, the remaining fields usually appear in order.
        # We'll attempt a best-effort extraction by removing the date strings.
        parts_wo_dates = [p for p in parts if p not in set(dates)]

        # Try to map:
        # city, address, zip, county, trustee
        ...
        out.append(
            {
                "sale_date_iso": _parse_mmddyyyy(sale_date_raw or ""),
                "continuance_date_iso": _parse_mmddyyyy(cont_date_raw or ""),
                "city": city,
                "address": address,
                "zip": zip_code,
                "county": county,
...
def run():
    ...
    created = updated = 0
    skipped_expired = skipped_lt30 = skipped_kill = skipped_no_date = 0
    wrote_count = 0

    # The site has pagination "1 2 3 4" and "Page size".
    # We'll just try ?Page=2 style first; if it fails, we still get page 1 data.
    # If the site doesn't support query params, we still harvest page 1 tonight.
    for page in range(1, FORECLOSURE_TN_MAX_PAGES + 1):
        url = FORECLOSURE_TN_SEED_URL
        if page > 1:
            # common pattern for ASPX data grids
            url = f"{FORECLOSURE_TN_SEED_URL}?page={page}"
        ...
        for r in rows:
            # choose the effective sale date:
            # if continuance exists, that's the forward-looking date
            sale_date_iso = r["continuance_date_iso"] or r["sale_date_iso"]
            if not sale_date_iso:
                skipped_no_date += 1
                continue

            county = r["county"] or "TN"
            address = r["address"]
            trustee = r["trustee"]
            ...
            props = build_properties(
                ...
                contact_info=trustee or (reason or ""),
    ...
    print(
        ...
        f"skipped_lt30={skipped_lt30} skipped_kill={skipped_kill}"
    )
