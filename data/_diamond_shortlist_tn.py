...
print("DIAMOND_SHORTLIST_TN (NO NEW ATTOM CALLS)")
print("RULES: status=enriched, auction_readiness=GREEN, dts_days 21-60, avm_low>=300k, spread_pct<=0.18")
...
for r in rows:
    ...
    try:
        ...
        merged=False
    out.append((lo, lk, county, dts, score, round(sp,4), merged, addr, ts))

print("CANDIDATES", len(out))
for lo, lk, county, dts, score, sp, merged, addr, ts in sorted(out, reverse=True)[:25]:
    print(" -", {"lead_key":lk,"county":county,"avm_low":lo,"dts_days":dts,"score":score,"spread_pct":sp,"has_detail":merged,"address":addr,"enriched_at":ts})
